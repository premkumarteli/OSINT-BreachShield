...
r_foot = p_foot.add_run("*This paper documents the design, engineering evolution, mathematical formulations, and experimental results of Major Project Phase II (BIS786) by Batch 05, Department of Information Science & Engineering, Acharya Institute of Technology, affiliated with Visvesvaraya Technological University (VTU), Belagavi, Karnataka, India.")
...
def add_table_clean(headers, rows, caption, col_widths=None, font_size=6.8):
    ...
    tblBorders = parse_xml(f'<w:tblBorders {nsdecls("w")}><w:top w:val="single" w:sz="6" w:space="0" w:color="000000"/><w:bottom w:val="single" w:sz="6" w:space="0" w:color="000000"/><w:insideH w:val="single" w:sz="4" w:space="0" w:color="D3D3D3"/><w:left w:val="none"/><w:right w:val="none"/><w:insideV w:val="none"/></w:tblBorders>')
...
def add_algorithm(number, title, inputs, outputs, lines):
    ...
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    tblPr = table._tbl.tblPr
    tblBorders = parse_xml(f'<w:tblBorders {nsdecls("w")}><w:top w:val="single" w:sz="8" w:space="0" w:color="000000"/><w:bottom w:val="single" w:sz="8" w:space="0" w:color="000000"/><w:insideH w:val="none"/><w:left w:val="none"/><w:right w:val="none"/><w:insideV w:val="none"/></w:tblBorders>')
    tblPr.append(tblBorders)
...
rk2.font.name = 'Times New Roman'; r2.font.size = Pt(9)
...
add_p("• Design of a tamper-evident audit logging engine utilizing canonical JSON serialization, binary Merkle tree batching, and local EVM smart contract anchoring with crash-recovery dead-letter queuing.")
add_p("• Verification across 94 automated backend tests and comprehensive performance profiling demonstrating sub-180 ms end-to-end lookup latency.")
...
# SECTION III: SYSTEM DESIGN & ARCHITECTURAL EVOLUTION
add_heading1("III. SYSTEM DESIGN & ARCHITECTURAL EVOLUTION")
add_heading2("A. Conceptual Phase 1 Scope vs. Phase 2 Realization")
add_p("In our Phase 1 project report, our initial proposal broadly envisioned a multi-faceted cybersecurity suite comprising a client browser extension for phishing detection, a website vulnerability scanner, external threat feed integrations (such as VirusTotal and PhishTank), and public blockchain logging. However, as our team advanced into Phase 2 development and tested initial prototypes, empirical constraints required rational engineering refinements:")
add_p("1) Resolving the Core Privacy Challenge: Attempting to build a client browser extension and generic vulnerability scanner simultaneously diluted engineering resources from solving the central vulnerability in breach detection: query privacy. We therefore concentrated our development on an end-to-end zero-knowledge client-side k-anonymity protocol and a robust out-of-band identity verification gate [11], [12].")
add_p("2) OSINT Sourcing Realities: While Phase 1 proposed general third-party security APIs, real-world testing exposed strict rate-limits, expensive API keys, and delayed disclosures. We adapted the OSINT layer to incorporate a high-concurrency Telegram scraper using Telethon MTProto, extracting intelligence directly from underground channels where freshly dumped credential lists are actively traded [5], [17].")
add_p("3) Blockchain Gas Economics & Determinism: Initial tests anchoring every log directly to public testnets (e.g., Polygon Amoy) encountered unpredictable RPC timeouts, faucet token exhaustion, and fluctuating gas fees that caused CI pipeline failures. We rationalized our architecture by deploying an automated local Hardhat EVM node with automatic boot deployment, ensuring deterministic, zero-cost test execution while maintaining complete Solidity smart contract portability [4], [9], [18].")
...
add_heading2("B. Four-Tier Monorepo Architecture")
...
add_p("2) Backend API Gateway: An Express 5 Node.js orchestration engine managing dual-channel OTP generation and verification, signed JWT session tokens, 403 Forbidden search gating, MySQL 8.0 storage with an automatic JSON file fallback layer, and a pluggable breach source registry [11], [15].")
...
# SECTION IV: PRIVACY-PRESERVING PROTOCOL & OSINT HARVESTING
add_heading1("IV. PRIVACY-PRESERVING PROTOCOL & OSINT HARVESTING")
...
# SECTION V: MACHINE LEARNING THREAT SEVERITY TRIAGE
add_heading1("V. MACHINE LEARNING THREAT SEVERITY TRIAGE")
add_heading2("A. Benchmark Dataset Curation & Preprocessing")
add_p("In Phase 1, our survey highlighted the need for automated breach risk scoring [1], [7]. In Phase 2, our team curated a benchmark dataset of 1,034 verified enterprise breach incidents from Have I Been Pwned and historical threat repositories. Each record contains DataClasses (types of compromised data), PwnCount (compromised account volume), and IsVerified. Domain security experts labeled each incident into four severity tiers: LOW, MEDIUM, HIGH, and CRITICAL. The dataset was partitioned using stratified 70/15/15 train/val/test splits as summarized in Table II [13].")
...
add_table_clean(
    ...
    "TABLE II. DATASET PARTITION & SEVERITY DISTRIBUTION",
...
add_p("Test set results are reported in Table III, Table IV, Fig. 4, and Fig. 5.")
...
add_table_clean(
    ...
    "TABLE III. COMPARATIVE BENCHMARK ON TEST SET (N=157)",
...
add_table_clean(
    ...
    "TABLE IV. PER-CLASS PERFORMANCE METRICS ACROSS SEVERITY TIERS",
...
add_heading2("A. Canonical Merkle Tree Construction & EVM Smart Contract Anchoring")
...
print(f"Comprehensive 7-9 page IEEE paper regenerated successfully at: {output_docx}")
